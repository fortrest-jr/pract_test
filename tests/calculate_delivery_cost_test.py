...
def test_counting_cost_on_middle_values(distance: float, size: float, fragile: bool, workload: Workload, expected_cost:
float):
    result_cost = calculate_delivery_cost(distance, size, fragile, workload)
    assert result_cost == pytest.approx(expected_cost)


def get_value_cost_pairs_from_interval_dict(interval_dict: dict[T:(int, bool, Workload), int]) -> list[
    value_cost[T, int]]:
    return [value_cost(value, cost) for value, cost in interval_dict.items()]


all_params_value_costs = [
    all_params_value_cost
    for all_params_value_cost in AllPairs(
        [
...
def test_counting_cost_on_boundaries(
        distance: value_cost[int, int],
        size: value_cost[int, int],
        fragile: value_cost[bool, int],
        workload: value_cost[Workload, float]):
...
def test_counting_cost_returns_minimal_cost_when_lower_is_calculated(
        distance: float, size: float, fragile: bool, workload: Workload
):
    result_cost = calculate_delivery_cost(distance, size, fragile, workload)
